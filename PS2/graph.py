...
def get_best_path(digraph, start, end, path, max_dist_outdoors, best_dist,
    ...
    if start not in digraph.nodes or end not in digraph.nodes:
        raise ValueError('nodes are not  in the graph')
    if start==end:
        return path
    else:
        ...
        for node in digraph.get_edges_for_node(start):
            ...
            if node.dest not in path:
                newPath=get_best_path(digraph,node.dest,end,path,max_dist_outdoors,best_dist,best_path)
                if newPath != None:
