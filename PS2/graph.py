...
class WeightedEdge(Edge):
    ...
    def __str__(self):
        return self.src.get_name()+'->'+self.dest.get_name() + ' ('+str(self.get_total_distance())+', '+ str(self.get_outdoor_distance()) + ')'
...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        if edge.get_source() not in self.nodes or edge.get_destination() not in self.nodes:
            raise ValueError('Nodes associated with edges are not in the graph')
